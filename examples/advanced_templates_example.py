...
from easypptx import Presentation, Text, Image, Chart, Pyplot
...
title_slide = pres.add_title_slide(
    title="Advanced Templates in EasyPPTX",
    subtitle="Showcasing specialized slide layouts"
)
...
# Update the title
team_title = [shape for shape in team_slide.shapes if shape.has_text_frame][0]
...
for i, (position, name, title) in enumerate(zip(member_positions, member_names, member_titles)):
    ...
    try:
        ...
        placeholder = team_slide.add_shape(
            position=position,
...
statement_shape = [shape for shape in statement_slide.shapes if shape.has_text_frame][0]
...
# Update the title
dashboard_title = [shape for shape in dashboard_slide.shapes if shape.has_text_frame][0]
...
revenue = [random.randint(900, 1500) for _ in range(12)]
expenses = [random.randint(700, 1200) for _ in range(12)]
...
for i, step in enumerate(steps):
    ...
    Text.add(
        slide=timeline_slide,
        text=step["date"],
        position={"x": x, "y": date_y, "width": step_width, "height": str(float(step_height.replace("%", ""))/4) + "%"},
        font_name="Meiryo",
        font_size=12,
        align="center",
        vertical_align="middle",
    ...
    Text.add(
        ...
        position={"x": x, "y": desc_y, "width": step_width, "height": str(float(step_height.replace("%", ""))/4) + "%"},
        font_name="Meiryo",
        font_size=10,
        align="center",
        vertical_align="middle",
    ...
    if i < len(steps) - 1:
        ...
        connector_width = str(100 - 2*float(timeline_area["x"].replace("%", "")) - 2*float(step_width.replace("%", ""))) + "%"
